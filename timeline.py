...
class Timeline:
    ...
    def add_job(self, app_type: str, lifetime_seconds: int) -> Job:
        """
        ...
        with self._lock:
            ...
            for job in self._jobs:
                if job.status == "queued" and job.start_tick <= tick:
                    job.status = "running"
                    logger.info(f"Job started: {job.job_id} | app={job.app_type}")

            # remove expired jobs
            before = len(self._jobs)
            expired = [j for j in self._jobs if j.is_expired(tick)]
            for job in expired:
                job.status = "done"
                logger.info(f"Job expired: {job.job_id} | app={job.app_type}")
