...
Generates emulation-ready pod-level datasets for all combinations:
  hotel: 0-8  ×  sn: 0-7  ×  sa: 0-6  ×  es: 0-5  =  3,024 combinations

Strategy per combination:
  h0s0a0e0        → baseline idle dataset (from baseline_node.csv)
...
ES support:
  - ES scales exactly like hotel/sn/sa: es{N} = N separate namespaces
    ...
    (confirmed empirically — not a single pod's load level).
  - No standalone ES experiment exists in the raw data, so h0s0a0eN targets
    fall back to the idle baseline as their base (0 replicas of everything),
    then add N ES replicas on top.
  - ES is RAM-dominant: a generated combo's total RAM is checked against
    ...
    exceed physical node capacity (this can happen once ES is stacked with
    other apps — real es5-inclusive experiments already reach ~82% of
    capacity, e.g. sn5_es5).
...
Power: pod_cpu_watts = 0.00241×cpu + 0.0000312×ram + 0.073
Disk: pod disk_space_mb + OS baseline floor

Usage:
    python prepare_datasets_v2.py
        --corrected ./corrected_full
        --raw       ./all_data_full
        --baseline  ./all_data_full/baseline_node.csv
        --out       ./datasets
"""
...
def estimate_node_psi(pod_df: pd.DataFrame) -> pd.Series:
    ...
    Cap enforced at cpu_utilization × NODE_PSI_MAX_US.
    """
    results = {}
...
def estimate_node_sched(
    ...
    Two-component sched estimation:
      node_sched = OS_sched + app_sched × (new_psi / base_psi)
...
def make_idle_dataset(baseline_path: str, out_dir: str) -> Dict:
    """Generate h0s0a0e0 from baseline_node.csv."""
    out_df = load_idle_base_df(baseline_path)
    out_path = os.path.join(out_dir, 'h0s0a0e0_pod.csv')
    out_df.to_csv(out_path, index=False)
    logger.info(f"  h0s0a0e0: idle dataset saved ({len(out_df)} windows)")
    return {
        'file': f'datasets/h0s0a0e0_pod.csv',
        'source': 'baseline',
        'hotel': 0, 'sn': 0, 'sa': 0, 'es': 0,
        'windows': len(out_df), 'pods': 1,
        'tier': 'A',
    }
...
def make_direct_dataset(
    ...
    out_path = os.path.join(out_dir, f"{key}_pod.csv")
    df.to_csv(out_path, index=False)

    return {
        'file':    f'datasets/{key}_pod.csv',
        'source':  'real',
...
def main():
    parser = argparse.ArgumentParser(description="Generate emulation datasets v2 (with ES)")
    parser.add_argument('--corrected', default='./corrected_full',
                        help='Directory with corrected pod CSVs + experiment_meta.json')
    parser.add_argument('--raw',       default='./all_data_full',
                        help='Directory with raw experiment CSVs')
    parser.add_argument('--baseline',  default='./all_data_full/baseline_node.csv',
                        help='Baseline node CSV')
    ...
    # load metadata
    meta_path = os.path.join(args.corrected, 'experiment_meta.json')
    with open(meta_path) as f:
        meta = json.load(f)
    tier_A = meta['tier_A']
    tier_B = meta['tier_B']
    logger.info(f"Tier A: {len(tier_A)} | Tier B: {len(tier_B)}")

    # load single-instance templates for scaling
    logger.info("Loading replica templates...")
    templates = {}
    for app, tmpl_name in [('hotel', 'hotel1'), ('sn', 'sn1'), ('sa', 'sa5')]:
        path = os.path.join(args.corrected, f"{tmpl_name}_pod_corrected.csv")
    ...
    es_source_path = os.path.join(args.corrected, "hotel1_es1_pod_corrected.csv")
    ...
    # build lookup of available corrected experiments
    available = set(
        f.replace('_pod_corrected.csv', '')
        for f in os.listdir(args.corrected)
        if f.endswith('_pod_corrected.csv')
    )

    dataset_index = {}
    skipped = []
    real_count = generated_count = 0
    ram_capped_count = 0

    # ── h0s0a0e0: idle ────────────────────────────────────────────────────────
    logger.info("Generating h0s0a0e0 (idle baseline)...")
    entry = make_idle_dataset(args.baseline, args.out)
    dataset_index['h0s0a0e0'] = entry

    # ── full grid ─────────────────────────────────────────────────────────────
    total = 9 * 8 * 7 * 6
    done  = 0
    for h in range(0, 9):      # 0-8
        for s in range(0, 8):  # 0-7
            for a in range(0, 7):  # 0-6
                for e in range(0, 6):  # 0-5
                    if h == 0 and s == 0 and a == 0 and e == 0:
                    ...
                    if exact_A:
                        entry = make_direct_dataset(exact_A, h, s, a, e, args.corrected, args.out)
                        if entry:
                            dataset_index[key] = entry
                            real_count += 1
                            if entry.get('ram_capped'):
                                ram_capped_count += 1
                            if done % 200 == 0:
                                logger.info(f"  [{done}/{total}] {key} → real ({exact_A})")
                            continue

                    # find closest Tier A base (or idle fallback)
                    base_exp = find_best_base(h, s, a, e, tier_A)
                    if base_exp is None:
                        logger.warning(f"  {key}: no Tier A base found — skipping")
                        skipped.append(key)
                        continue

                    entry = make_generated_dataset(
                        h, s, a, e, base_exp, args.corrected, args.out, args.baseline, templates
                    )
                    if entry:
                        dataset_index[key] = entry
                        generated_count += 1
                        if entry.get('ram_capped'):
                            ram_capped_count += 1
                        if done % 200 == 0:
                            logger.info(f"  [{done}/{total}] {key} → generated (base={base_exp})")
                    else:
                        skipped.append(key)

    # save index
    index_path = os.path.join(args.out, 'dataset_index.json')
    with open(index_path, 'w') as f:
        json.dump(dataset_index, f, indent=2)

    logger.info(f"\n{'='*60}")
    logger.info(f"Done.")
    logger.info(f"  Total datasets:    {len(dataset_index)}")
    logger.info(f"  Real (Tier A):     {real_count + 1}")  # +1 for idle
    logger.info(f"  Generated:         {generated_count}")
    logger.info(f"  RAM-capped:        {ram_capped_count}")
    logger.info(f"  Skipped:           {len(skipped)}")
    if skipped:
        logger.info(f"  Skipped keys:      {skipped}")
